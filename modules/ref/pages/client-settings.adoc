...
[source,go]
----
include::devguide:example$configuration.go[tags=configuration]
----
...
|`10m` (10 minutes)
|Default query execution time limit for executing the query when no deadline is applied by `context.Context` at the query level.
|===
...
This cluster option specifies the _default_ unmarshaler.
You can override the unmarshaler for a specific query by setting the `Unmarshaler` query option when executing the query.
