...
One of Couchbase's strengths is speedy response, so deployment of apps should be in the same region as your Analytics cluster.

We always recommend the xref::analytics-sdk-release-notes.adoc#latest-release[latest version] of the SDK. 
This not only contains the latest security updates and bug fixes, but will be compatible with the latest Enterprise Analytics release.
// -- Capella Analytics always runs a recent version of Enterprise Analytics.
...
// For the Go Analytcis SDK, that is to keep a separate properties file for your development and production environments.
